...
class TeamsProcessor:
    ...
    async def _initialize_graph_client(self) -> None:
        ...
        if not all([tenant_id, client_id, client_secret]):
            raise ValueError(
                "Mangler påkrevde miljøvariabler: TENANT_ID, CLIENT_ID, eller CLIENT_SECRET"
            )
        ...
        self.planner_manager = PlannerTemplateManager(
            self.graph_client, self.credentials
        )
    ...
    async def process_team(self, team) -> Optional[Dict]:
        ...
        if not self.is_initialized:
            raise RuntimeError(
                "TeamsProcessor er ikke initialisert. Kall initialize() først."
            )
        ...
        try:
            ...
            while teams_response:
                ...
                if teams_response.odata_next_link:
                    teams_response = await self.graph_client.teams.with_url(
